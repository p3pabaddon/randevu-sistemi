...
function playNotificationSound() {
    try {
        ...
        notes.forEach((freq, i) => {
            ...
            gain.gain.linearRampToValueAtTime(0.28, ctx.currentTime + i * 0.13 + 0.02);
...
slugInput.addEventListener('keydown', (e) => { if (e.key === 'Enter') doLogin(); });
  ...
  <div class="appointment-card status-${a.status} ${expired ? 'expired' : ''}"
    ...
    <div class="card-actions">
      <button class="btn btn-ghost btn-sm whatsapp-quick-btn"
              onclick="event.stopPropagation(); quickWhatsApp('${esc(a.customer_phone)}','${esc(a.customer_name)}','${formatDate(a.appointment_date)}','${a.appointment_time?.slice(0, 5)}','${esc(svcName)}')"
...
function initModal() {
    ...
    modalWA.addEventListener('click', () => {
        ...
        quickWhatsApp(a.customer_phone, a.customer_name, formatDate(a.appointment_date), a.appointment_time?.slice(0, 5), a.services?.name || '–');
    });
...
// ── WHATSAPP ──────────────────────────────────────────────────────────────────
function quickWhatsApp(phone, name, date, time, service) {
    const num = phone.replace(/\D/g, '');
    const msg = encodeURIComponent(`Merhaba ${name}!\n${service} için ${date} ${time} randevunuz onaylandı.\nBizi tercih ettiğiniz için teşekkürler.`);
...
function setMinDate() {
    const d = $('f-date');
    if (d) { d.min = today(); d.value = today(); }
}
...
function initRealtime() {
    try {
        ...
        if (!createClient) throw new Error('no CDN');
        const sb = createClient(SUPABASE_URL, SUPABASE_ANON_KEY);
        sb.channel('appointments-changes')
            .on('postgres_changes', {
                event: '*', schema: 'public', table: 'appointments',
                filter: `tenant_id=eq.${state.tenantId}`,
            }, (payload) => {
                // Yeni randevu gelince bildirim sesi çal
                if (payload.eventType === 'INSERT') {
                    playNotificationSound();
                    showToast(`Yeni randevu: ${payload.new.customer_name} — ${payload.new.appointment_time?.slice(0, 5)}`);
                }
                if (state.activeTab === 'today') loadToday();
            ...
            .subscribe((status) => {
                if (status === 'SUBSCRIBED') {
                    connStatus.textContent = 'CANLI'; connStatus.className = 'connection-status connected';
                } else if (status === 'CHANNEL_ERROR' || status === 'CLOSED') {
                    connStatus.textContent = 'Bağlantı Kesildi'; connStatus.className = 'connection-status disconnected';
                }
            });
    } catch (_) {
        // Polling fallback (30 saniye)
        connStatus.textContent = 'Polling'; connStatus.className = 'connection-status';
        setInterval(async () => {
            const data = await fetchAppointments({ date: today() });
