...
router.post('/', async (req, res) => {
    const { name, slug, phone, email, address, whatsapp_number } = req.body;
    if (!name || !slug) return res.status(400).json({ error: 'name ve slug zorunludur.' });

    const { data, error } = await supabase
        .from('tenants')
        .insert([{ name, slug, phone, email, address, whatsapp_number }])
        .select()
        .single();
...
router.patch('/:id', async (req, res) => {
    const allowed = ['name', 'phone', 'email', 'address', 'whatsapp_number'];
    const updates = {};
