...
function switchTab(key) {
    ...
    if (key === 'crm') loadCRM();
    if (key === 'new') { loadServices(); renderCampaignServicesList(); }
    if (key === 'services') loadServices();
    if (key === 'reports') { loadReports('all'); updateBentoStats(); }
...
if ($('services-refresh-btn')) $('services-refresh-btn').addEventListener('click', fetchServices);
...
function _connectSSE() {
    ...
    setInterval(async () => {
        const data = await fetchAppointments({ date: today() });
        const oldToday = _todayAllAppts || [];

        if (_prevApptCount !== null) {
            const newAppts = data.filter(a => !oldToday.find(oa => oa.id === a.id));
        ...
        if (state.activeTab === 'today') {
            ...
            renderAppointments('today-appointments', data);
            updateStats(data);
        }
        updateBentoStats();
    }, 10_000);
...
function renderDeletedAppointments(list) {
    ...
    el.innerHTML = list.map(a => {
        ...
        return `
        <div class="appointment-card deleted" style="border-left:4px solid var(--red); cursor:pointer; transition:all 0.2s;"
             onclick="openDeletedDetail(${JSON.stringify(a).replace(/"/g, '&quot;')})"
             onmouseenter="this.style.transform='translateY(-2px)'; this.style.boxShadow='0 8px 25px rgba(0,0,0,0.3)';"
            ...
            <div style="display:flex; align-items:center; justify-content:space-between; flex-wrap:wrap; gap:0.5rem;">
                <div style="font-size:0.72rem; color:var(--text-muted); display:flex; align-items:center; gap:0.4rem;">
                    ...
                    Kalıcı silme için <strong>${hoursLeft}s ${minsLeft}dk</strong> kaldı.
                </div>
                <button class="btn btn-ghost"
                    style="padding:0.3rem 0.85rem; font-size:0.78rem; border-radius:var(--radius-sm); border:1px solid var(--green); color:var(--green); background:rgba(34,197,94,0.05);"
                    onclick="event.stopPropagation(); window.restoreAppointment('${a.id}')">
                    ↩️ Geri Getir
                </button>
            </div>
        </div>`;
...
window.openDeletedDetail = function (a) {
    let overlay = $('deleted-detail-overlay');
    if (!overlay) {
        overlay = document.createElement('div');
        overlay.id = 'deleted-detail-overlay';
        overlay.className = 'reset-overlay';
        overlay.style.cssText = 'z-index:10500;';
        overlay.innerHTML = `
            <div class="reset-card" style="max-width:480px; max-height:85vh; overflow-y:auto;">
                <button class="reset-close-btn" id="deleted-detail-close">✕</button>
                <div id="deleted-detail-body"></div>
            </div>`;
        document.body.appendChild(overlay);
        overlay.querySelector('#deleted-detail-close').onclick = () => overlay.classList.add('hidden');
    }
    overlay.classList.remove('hidden');

    const servicePrice = a.services?.discounted_price != null
        ? `₺${Number(a.services.discounted_price).toLocaleString('tr-TR')} <span style="text-decoration:line-through; opacity:0.5; font-size:0.8em;">₺${Number(a.services?.price || 0).toLocaleString('tr-TR')}</span>`
        : (a.services?.price ? `₺${Number(a.services.price).toLocaleString('tr-TR')}` : '—');

    $('deleted-detail-body').innerHTML = `
        <div style="margin-bottom:1.25rem;">
            <h2 style="margin-bottom:0.25rem;">🗑️ Silinen Randevu</h2>
            <span style="font-size:0.7rem; background:rgba(239,68,68,0.1); color:var(--red); padding:0.2rem 0.5rem; border-radius:4px; font-weight:700;">SİLİNDİ</span>
        </div>
        <div style="display:grid; grid-template-columns:1fr 1fr; gap:0.75rem; margin-bottom:1.25rem;">
            <div style="padding:0.75rem; background:var(--bg-deep); border-radius:var(--radius-sm);">
                <div style="font-size:0.7rem; color:var(--text-muted); margin-bottom:0.25rem;">MÜŞTERİ</div>
                <div style="font-weight:700;">${esc(a.customer_name)}</div>
                <div style="font-size:0.8rem; color:var(--text-muted);">${esc(a.customer_phone)}</div>
            </div>
            <div style="padding:0.75rem; background:var(--bg-deep); border-radius:var(--radius-sm);">
                <div style="font-size:0.7rem; color:var(--text-muted); margin-bottom:0.25rem;">HİZMET</div>
                <div style="font-weight:700;">${esc(a.services?.name || '—')}</div>
                <div style="font-size:0.8rem; color:var(--accent);">${servicePrice}</div>
            </div>
            <div style="padding:0.75rem; background:var(--bg-deep); border-radius:var(--radius-sm);">
                <div style="font-size:0.7rem; color:var(--text-muted); margin-bottom:0.25rem;">TARİH & SAAT</div>
                <div style="font-weight:700;">${formatDate(a.appointment_date)}</div>
                <div style="font-size:0.8rem; color:var(--text-muted);">${a.appointment_time?.slice(0, 5)}</div>
            </div>
            <div style="padding:0.75rem; background:var(--bg-deep); border-radius:var(--radius-sm);">
                <div style="font-size:0.7rem; color:var(--text-muted); margin-bottom:0.25rem;">PERSONEL</div>
                <div style="font-weight:700;">${esc(a.staff?.name || 'Belirsiz')}</div>
            </div>
        </div>
        ${a.notes ? `<div style="padding:0.75rem; background:var(--bg-deep); border-radius:var(--radius-sm); margin-bottom:1.25rem;"><div style="font-size:0.7rem; color:var(--text-muted); margin-bottom:0.25rem;">NOTLAR</div><div style="font-size:0.85rem;">${esc(a.notes)}</div></div>` : ''}
        <button class="btn btn-primary" style="width:100%; background:var(--green); border-color:var(--green);"
            onclick="window.restoreAppointment('${a.id}').then(()=>{ document.getElementById('deleted-detail-overlay').classList.add('hidden'); })">
            ↩️ Geri Getir
        </button>`;
    if (window.lucide) lucide.createIcons();
};

window.restoreAppointment = async function (id) {
    try {
        ...
        const json = await handleResponse(res, true);
        showToast(json.message || 'Randevu geri getirildi. ❤️');
...
async function loadCampaigns() {
    // Deprecated, content merged into new tab
}

function renderCampaignServicesList() {
    const el = $('campaign-services-list');
    if (!el) return;
    if (!state.services || !state.services.length) {
        loadServices().then(() => {
            if (!state.services || !state.services.length) {
                el.innerHTML = '<div class="loading-state">Henüz hizmet eklenmemiş.</div>';
            } else renderCampaignServicesList();
        });
        return;
    }
    el.innerHTML = state.services.map(s => {
        const hasActiveCampaign = s.campaign_ends_at && new Date(s.campaign_ends_at) > new Date();
        return `
        <div class="appointment-card" style="cursor:pointer; display:flex; justify-content:space-between; align-items:center; flex-wrap:wrap; gap:0.5rem;"
             onclick="openCampaignEdit(${JSON.stringify(s).replace(/"/g, '&quot;')})">
            <div>
                <div style="font-weight:700; font-size:0.95rem;">${esc(s.name)}</div>
                <div style="font-size:0.78rem; color:var(--text-muted);">₺${Number(s.price || 0).toLocaleString('tr-TR')} • ${s.duration_minutes || '?'} dk</div>
            </div>
            ${hasActiveCampaign
                ? `<span style="font-size:0.72rem; background:rgba(201,168,76,0.15); color:var(--accent); padding:0.25rem 0.6rem; border-radius:9999px; font-weight:700;">🏷️ ${esc(s.campaign_label || 'Kampanya')} • ${new Date(s.campaign_ends_at).toLocaleDateString('tr-TR')}</span>`
                : `<span style="font-size:0.72rem; background:var(--bg-deep); color:var(--text-muted); padding:0.25rem 0.6rem; border-radius:9999px;">Kampanya Yok</span>`
            }
        </div>`;
    }).join('');
}
